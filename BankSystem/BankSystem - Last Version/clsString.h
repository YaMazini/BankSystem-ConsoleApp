...
public:
    ...
    static string Encrypt(string S1, short EncryptionKey = 2)
    {
        string EncryptedString = "";

        for (short i = 0; i < S1.length(); i++)
        {
            EncryptedString += char((int)S1[i] + EncryptionKey);
        }

        return EncryptedString;
    }

    static string Decrypt(string S1, short DecryptionKey = 2)
    {
        string DecryptedString = "";

        for (short i = 0; i < S1.length(); i++)
        {
            DecryptedString += char((int)S1[i] - DecryptionKey);
        }
